 ...
 * приходили новые.
 *
 * В 1973 г. мир увидел язык Си (автор Деннис Ритчи),
...
/* Уже так страшно, правда?
 *
 * C++ автоматически выполняет огромное количество 
 * команд, поднимая программиста на более высокий уровень
 * абстракции, что позволяет "не забивать голову лишним",
 * Это так же значительно улучшает читаемость программ.
 * Однако, если это необходимо, программист может
 * получить доступ к низкому уровню прямо из C++.
 *
 * Нельзя не уточнить, что С++ является компилируемым
 * языком. Это означает, что перед выполнением программа
 * преобразуется в машинные коды (если нет ошибок).
 * Выполнение программы в машинных кодах происходит 
 * максимально быстро, а вот скорость компиляции
 * зависит от размера программы. 
 *
...
// а текст после двух слэшей - однострочным 

/* Комментарий - это заметка для себя или других
 * программистов. При компиляции они игнорируются.
 * Дополнительные звёздочки в начале каждый строки - 
 * просто хороший тон, они не обязательны. */
 ...
 * (InputOutputSTREAM) 
 *
 * Библиотека - готовый программный код, который ты можешь
 * использовать в своих целях. Например, библиотека cmath 
 * даёт доступ к функциям sin(), cos(), max() и др. */
 ...
 * 0 в Операционных Системах (далее ОС) означает, что
 ...
 * на следующую строку. 
 *
 * Теперь ты понимаешь, как работает простая 
 * программа. На протяжении достаточно долгого времени весь
 * твой код будет сосредоточен */
 ...
 * Теперь для тебя это не просто непонятные слова :)
 * До встречи в Главе 2 */
