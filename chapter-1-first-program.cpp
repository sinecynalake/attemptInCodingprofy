 ...
 * перед выполнением пройти этап компиляции.
 * Это процесс преобразования "читаемого кода"
 * в машинный язык, который непосредственно выполнятеся 
 * процессором. То есть мы пишем программу не думая
 * об операциях, которые происходят внутри компьютера.
 * Нас интересует результат. А код преобразуется 
 * в машинный с помощью компилятора. 
 * Ясненько?
 * Надеюсь, что да :) 
 *
